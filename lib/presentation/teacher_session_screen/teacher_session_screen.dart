...
class _TeacherSessionScreenState extends State<TeacherSessionScreen>
    ...
    try {
      ...
      if (existingSession != null) {
        ...
        if (_durationSeconds > 400) {
          ...
          await _loadClassStudents(existingSession.classId);
  ...
  Future<void> _loadClassStudents(String classId) async {
    try {
      final students = await SupabaseService.listUsers(role: 'student');
      if (mounted) {
        setState(() {
          _classStudents = students.where((s) => s.classId == classId).toList();
        });
      }
    } catch (e) {
  ...
  Future<void> _startSession() async {
    ...
    try {
      ...
      if (session != null) {
        ...
        await _loadClassStudents(session.classId);
